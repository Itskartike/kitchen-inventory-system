...
export const useCloudKitchen = () => {
  ...
  if (!context) {
    throw new Error(
      "useCloudKitchen must be used within a CloudKitchenProvider"
    );
  }
  return context;
};
...
export const CloudKitchenProvider = ({ children }) => {
  const [selectedKitchen, setSelectedKitchen] = useState(() => {
    const savedKitchen = localStorage.getItem("selectedKitchen");
    return savedKitchen ? JSON.parse(savedKitchen) : null;
  });
  ...
  useEffect(() => {
    if (currentSuperAdmin) {
      localStorage.setItem(
        "currentSuperAdmin",
        JSON.stringify(currentSuperAdmin)
      );
    } else {
  ...
  const authenticateUser = useCallback(
    (username, password) => {
      ...
      const user = CloudKitchenDB.authenticateUser(
        selectedKitchen.id,
        username,
        password
      );
  ...
  const authenticateSuperAdmin = useCallback((username, password) => {
    const superAdmin = CloudKitchenDB.authenticateSuperAdmin(
      username,
      password
    );
    if (superAdmin) {
  ...
  const deductInventoryFromRecipe = useCallback(
    (recipe) => {
